...
import { getSearchedStudents } from "../utils/students";
...
const SearchedUsers = () => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        const studentsData = await getSearchedStudents(search, page);
    ...
    <>
      {data?.docs?.length === 0 ? (
        <div className="w-full h-[80%] flex items-center justify-center ">
          <h1 className="text-lg font-bold">Aluno não encontrado</h1>
        </div>
      ) : (
